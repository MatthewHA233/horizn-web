...
function buildGridHtml(occupancyMap) {
  ...
  for (const section of HULL_SECTIONS) {
    ...
    for (let i = start; i <= end; i++) {
      if (occupancyMap.get(i)?.current) sectionOccupied++
    }

    // 判断某行是否全空（无当前持有者也无历史）
    const isRowEmpty = ({ cellStart, cellEnd }) => {
      for (let i = cellStart; i <= cellEnd; i++) {
        const e = occupancyMap.get(i)
        if (e?.current || e?.legacy?.length) return false
      }
      return true
    }

    const renderRow = ({ cellStart, cellEnd }) => {
      const cells = []
      for (let i = cellStart; i <= cellEnd; i++) {
        ...
        cells.push(`<div class="flex-1 h-8 rounded text-[11px] font-mono font-bold relative overflow-hidden flex items-center justify-center ${cellClass}">
          ${glow}<span class="relative">${numStr}</span>
        </div>`)
      }
      return `<div class="flex gap-1">${cells.join('')}</div>`
    }

    const ellipsisHtml = `<div class="text-center text-[11px] text-gray-600 tracking-widest py-0.5">· · ·</div>`
    ...
    for (const row of rows) {
      if (isRowEmpty(row)) {
        ...
        if (pendingEmpty > 0) rowsHtml += ellipsisHtml
        pendingEmpty = 0
  ...
  <div class="flex items-center justify-center gap-3 pt-1 pb-0.5 flex-wrap">
    ...
    <div class="flex items-center gap-1.5"><div class="w-3.5 h-3.5 rounded bg-gray-700/15 border border-dashed border-gray-500/40"></div><span class="text-[11px] text-gray-500">仅历史</span></div>
...
// ─── 视图 2：舷号列表（list）—— 仅展示在役分配（revoked_at === null）────────
function buildListHtml(assignments) {
  // 按舷号数字排序（打印版用数字顺序）
  ...
  const rows = sorted.map((a, index) => {
    const rowBg = index % 2 === 0 ? 'bg-gray-900/20' : 'bg-gray-800/40'
    const isLeft = !a.member_active

    const leftBadge = isLeft
      ? `<span class="text-[9px] text-red-400/70 flex-shrink-0">已离队</span>`
      : ''

    const qqHtml = a.qq_id
      ? `<span class="text-gray-400">${a.qq_nickname || 'QQ用户'}</span>
         <span class="font-mono">${a.qq_id}</span>
         ${a.qq_join_time ? `<span class="text-gray-600">|</span><span>入群 <span class="text-gray-400">${formatDate(a.qq_join_time)}</span></span>` : ''}`
      : '<span class="text-gray-600">未关联 QQ</span>'
    const hullDateHtml = a.assigned_at
      ? `<span class="text-gray-600">|</span><span>授舷 <span class="text-cyan-400/70">${formatDate(a.assigned_at)}</span></span>`
      : ''
    const subRow = `
    ...
    <div class="px-2 py-1.5 ${isLeft ? 'opacity-60' : ''} ${rowBg}">
      <div class="flex items-center justify-between gap-1">
        <div class="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          <span class="text-xs truncate text-white">${a.primary_name || a.player_id}</span>
          ${leftBadge}
          <span class="text-[10px] text-gray-500 font-mono">${a.player_id}</span>
        </div>
        <div class="flex items-center gap-1 flex-shrink-0">
          <span class="text-[10px] px-1.5 py-0.5 rounded bg-blue-600/20 text-blue-400">${a.hull_number}</span>
        </div>
      </div>
      ${subRow}
    </div>`
  ...
  <div class="m-2 rounded border border-gray-700/50 overflow-hidden bg-gray-900/50">
    <div class="px-2 py-1.5 bg-gray-700/20 border-b border-gray-700/50 flex items-center justify-between">
      ...
      <span class="text-[11px] text-gray-500">共 ${sorted.length} 个在役舷号</span>
    </div>
...
export async function GET(request) {
  ...
  try {
    ...
    if (view === 'grid' || view === 'list') {
      const { data: assignments, error: assignErr } = await sb.rpc('horizn_get_hull_occupancy')
      ...
      if (view === 'grid') {
        ...
        for (const a of assignments || []) {
          ...
          if (a.revoked_at === null) entry.current = a
          else entry.legacy.push(a)
      ...
      if (view === 'list') {
        const activeAssignments = (assignments || []).filter(a => a.revoked_at === null)
        const png = await screenshotHtml(buildListHtml(activeAssignments), 520)
        return new Response(png, { headers: { 'Content-Type': 'image/png', 'Cache-Control': 'no-store' } })
      }
    }
